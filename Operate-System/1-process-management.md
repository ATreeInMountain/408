...
* 抢占式 —— 如果由中断，或更高优先级进程进入，**可以让进程被动放弃处理机**
...
* 进程切换的主要步骤：
  ...
  1. 对正在运行的进程：保存旧PC、通用寄存器、堆栈等现场信息到它的PCB，修改程序状态字
  2. 对将要运行的进程：从它的PCB恢复PC、通用寄存器、堆栈等现场信息，修改程序状态字
...
* 调度时机：
  ...
  2. 被动放弃：
     ...
     * 创建新进程（可能会产生高优先级进程）
     * 中断（如I/O中断）
* 非抢占式—— **终止、阻塞** 时会触发调度程序工作
* 抢占式 —— **每个时钟中断** 都会触发调度程序工作（检查是否发生了终止、阻塞、时间片到、创建新进程、中断）
...
* 周转时间 —— 作业从提交到完成花费的时间
  ...
  周转时间 = 作业完成时刻 - 作业提交时刻 = 高级调度的时间 + 在就绪态的时间 + 在运行态的时间 + 在阻塞态的时间
