...
* 折半查找的判定树一定是 **完全二叉树** （只有最下边的一层是不满的）
...
* 在散列表中，查找长度 = 0 的情况是可能发生的（明确查找长度的定义）
...
* 平衡二叉树：左右子树高度之差不超过1
  ...
  * 对于更高的h，满足：
    ...
    * 所以，$n_h = n_{h-1} + n_{h-2} + 1$

  可以得出，一个高度为h的平衡二叉树的结点数目在 $[n_h , 2^h - 1]$ 之间。
  ...
  * 一个高为 5 的平衡二叉树的结点数目在 $[12, 31]$ 之间
  ...
  以上总结：**一个高度为h的平衡二叉树，至少有$n_h$个结点**

* **一个有$n_h$个结点的平衡二叉树，高度至少是 $ h=\lceil\log_2(n+1)\rceil$**
...
如果红黑树黑高是 h，求最少有多少个结点

最少情况肯定是没有红色结点，同时为了满足黑路同，这一定是一个满二叉树。

所以黑高为 h 的红黑树，最少有 $2^h - 1$ 个结点

![image-20250301132222895](imgs\image-20250301132222895.png)

#### 插入

![image-20250301125004254](imgs\image-20250301125004254.png)

> 新结点如果是非根节点则默认涂红，这样可以保证黑路同，而左根右，根叶黑也不会被破坏
>
> * 插入新结点如果破坏了红黑树结构，一定是破坏了 不红红
>
> * 爷变为新结点时，可能会破坏 不红红 和 根叶黑
...
* 有时树中存储的关键字过少，甚至不足 $\lceil{m/2}\rceil - 1$ 个，所以无法强制要求根结点的关键字个数
...
* 第 h+1 层计算出的那个结点数是该层的结点下限，n+1一定要大于这个下限，在这个要求下让h尽量的大
...
* 删除终端结点：
  ...
  2. 低于下限，兄弟够借，则兄弟传给父，父传给自己![image-20250301233524523](imgs\image-20250301233524523.png)
...
B+树和分块查找很类似，只是分块查找只有一个索引表，一个查找表；而B+树有多层索引表
