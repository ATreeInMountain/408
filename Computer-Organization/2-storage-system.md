...
* 主存、cache 都是易失性存储器，断电即失；磁盘光盘是非易失性存储器
* DRAM破坏性读出，SRAM非破坏性读出
...
* 易失性存储器：断电即失
* DRAM，SRAM都是 易失性存储器，但DRAM是破坏性读出存储器，SRAM是非破坏性读出存储器
...
* 选择刷新时间的三种方式
  1. **分散刷新**，每经过一个存取周期就刷新一次，刷新时无法存取，所以就相当于**存取周期翻倍**变成 10us
...
* 硬盘以**扇面**作为读写的基本单位
* 假设有 n 个磁盘，则总共有 **2(n-1) 个磁头**，因为一个磁盘有两个面可以存储数据，最上方和最下方的面不存数据
...
* 磨损均衡技术 —— 将“擦除”平均分布在每个块上
  ...
  * **动态磨损均衡** —— 每次写入时，总是**优先写入**到磨损少的块里
  * **静态磨损均衡** —— 定期检查磨损，把 读多写少的数据**移动**磨损较多的块 里
...
* cache**行** = cache**块** = 主存块 = 主存**页**

* **主存的数据地址 = 块号 + 块内地址**

  cache数据地址 = 块号 + 块内地址

  两者的块内地址长度相等，主存的块号长度比cache长
...
* 怎样实现 cache - 主存 块映射
  ...
  cache 里存 **主存块 + 标记（主存块号） + 有效位（块号是否有效）**
...
* 主存的某个块都可以放到cache的**任意**一个块中
* 一行cache = 有效位(1) + **标记(主存块号长度)** + 块(块内地址长度)
* 查询映射：查每一行cache的**有效位和标记**，命中后查该行对应的块

#### 2. 直接映射

![image-20250308181003776](imgs\image-20250308181003776.png)

* 主存的某个块只能放到cache的**一个块**中（主存块可以在cache中存储的**行号 = 主存块号 % cache总块号**）
* 主存块号 = **标记 + 行号** + 块内地址长度
* 一行cache = 有效位(1) + **标记(主存块号长度 - cache块号长度)** + 块(块内地址长度)
...
* n 路组相联映射 —— 一组里有 n 个cache行
* 主存的某个块只能放到cache的**一个组**中（主存块可以在cache中存储的**组号 = 主存块号 % 总组数**）
* 主存块号 = **标记 + 组号** + 块内地址长度
* 一行cache = 有效位(1) + **标记(主存块号长度 - 组号长度)** + 块(块内地址长度)
* 查询映射：根据映射规则**找到cache组号**，然后在组中依次查**有效位和标记**，命中后查该行对应的块
...
* 直接映射不需要考虑cache满的情况，因为只要访问到内存块，就直接写入对应的cache行
